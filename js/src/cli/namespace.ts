...
programCommand("update_namespace")
  .requiredOption(
    "-cp, --config-path <string>",
    "JSON file with mint, namespaceToken, and updates to one or many of: prettyName, permissivenessSettings, or whitelistedStakingMints"
  )
  .action(async (_files: string[], cmd) => {
    log.setLevel("info");
    const { keypair, env, rpcUrl, configPath } = cmd.opts();
    ...
    await anchorProgram.updateNamespace(
      {
      ...
      },
      {
        ...
        tokenHolder: walletKeyPair.publicKey,
      }
    );

    log.info(
      "Namespace Updated :: Run the show_namespace command if you want to see changes"
    );
  });
